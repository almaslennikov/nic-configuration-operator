...
var _ = Describe("NicDeviceReconciler", func() {
	...
	Describe("updateDeviceConfigInProgressStatusCondition", func() {
		It("should set the given status condition for device", func() {
			...
			err := reconciler.updateDeviceConfigInProgressStatusCondition(ctx, device, "TestReason", metav1.ConditionTrue, "test-message")
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(k8sClient.Get(ctx, k8sTypes.NamespacedName{Name: deviceName, Namespace: namespaceName}, device)).To(Succeed())
			err = reconciler.updateDeviceConfigInProgressStatusCondition(ctx, device, "AnotherTestReason", metav1.ConditionFalse, "")
			Expect(err).NotTo(HaveOccurred())
